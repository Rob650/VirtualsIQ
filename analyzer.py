...
WORD COUNT TARGETS: what_it_does=~500 words (4-5 detailed paragraphs) | who_is_behind_it=~125 words | what_is_notable=~125 words | risks_to_monitor=~125 words | market_opportunity=~125 words. Total ~1000 words.

{
  "what_it_does": "The centerpiece of the report — write ~500 words across 4-5 detailed paragraphs. Be specific, descriptive, and engaging. Use every piece of data provided. Do not be vague.\n\nPARAGRAPH 1 — Core product: What is this agent and what does it actually do? Describe the product or service in rich detail. What specific problem does it solve, and for whom? What makes it meaningfully different from a generic chatbot or traditional Web2 equivalent? Explain the core interaction model — how do users engage with it and what do they get out of it? Draw from the biography (AGENT_BIO), website content, and Twitter bio.\n\nPARAGRAPH 2 — Twitter and online presence: The agent's Twitter is AGENT_TWITTER. Based on the Twitter bio content provided, describe the account's personality, topics, and tone. If the bio references a Linktree, Discord, or other destinations, describe what those suggest about the project's community scope. Interpret follower count and engagement signals where available.\n\nPARAGRAPH 3 — Website and technology: Based on the website content provided (AGENT_WEBSITE_CONTENT), describe what the site highlights — features, pricing, documentation, or product demos. Then describe the technology stack: what platform or AI models does it run on, what is the role of Virtuals Protocol and Base L2, and what does AGENT_STATUS status mean technically?\n\nPARAGRAPH 4 — Token mechanics and use cases: How does the $AGENT_TICKER token tie into the product? Is it a governance token, access key, revenue share mechanism, or speculative asset? Walk through 2-3 concrete use cases where this agent provides clear, tangible value, and describe who the target user archetypes are.\n\nPARAGRAPH 5 — Current status: Is this agent currently live and usable? What is the verifiable evidence — Virtuals Protocol status (AGENT_STATUS), Twitter activity, trading volume ($AGENT_VOL)? What has shipped vs. what is still promised? What would a new user experience today?",
...
CRITICAL RULES:
1. Hit the word count targets: what_it_does must be ~500 words (4-5 paragraphs). Other sections ~125 words each. Total ~1000 words.
...
async def analyze_agent(agent_data: dict, model: str = None, top_ids: set = None) -> dict:
    ...
    try:
        ...
        message = client.messages.create(
            model=chosen_model,
            max_tokens=8192,
            system="You are a senior crypto research analyst at a fund specializing in AI agent ecosystems. You are bullish and fundamentals-focused — you look for what makes a project compelling, note risks briefly, and write with genuine conviction. Produce substantive, data-driven reports of ~1000 words total across 5 sections. Reference specific data points, URLs, and metrics provided. Every sentence must add new information. Never use filler phrases.",
