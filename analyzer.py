...
Analyze this AI agent from Virtuals Protocol and return a structured report in exactly 5 sections (~1000 words total: ~500 for what_it_does, ~125 words each for the other four sections).
...
WORD COUNT TARGETS: what_it_does=500 words minimum (3-4 paragraphs, the centerpiece) | who_is_behind_it=125 words minimum | what_is_notable=125 words minimum | risks_to_monitor=125 words minimum | market_opportunity=125 words minimum. Total ~1000 words minimum.

{
  "what_it_does": "Write approximately 500 words across 3-4 paragraphs. Be specific, descriptive, and engaging. Use the data provided. Do not be vague.\n\nPARAGRAPH 1 — Core product: What is this agent and what does it actually do? Describe the product or service in rich detail. What specific problem does it solve, and for whom? Why does this problem matter in the Web3/crypto context? What makes it meaningfully different from a generic chatbot, LLM wrapper, or traditional Web2 equivalent? Draw from the biography (AGENT_BIO), website content, and Twitter bio to paint a complete picture.\n\nPARAGRAPH 2 — Technology, token, and ecosystem: What platform or infrastructure does it run on? How does the Virtuals Protocol provide the autonomy layer — what does AGENT_STATUS status mean here technically? How does the $AGENT_TICKER token tie into the product — access key, governance, revenue share, or speculative asset? Reference on-chain mechanics where visible. Describe the Twitter account (AGENT_TWITTER) and website (AGENT_WEBSITE_CONTENT) as product surfaces.\n\nPARAGRAPH 3 — Users and current status: Who is this for? Walk through a realistic user journey. Name 2-3 concrete use cases with tangible value. Is the product live and usable right now — what is the verifiable evidence (AGENT_STATUS, trading volume $AGENT_VOL, website content)? What has shipped vs. what is still promised?\n\nPARAGRAPH 4 (optional, include only if there is meaningful additional data): Any notable integrations, roadmap items, or community signals not covered above.",
...
CRITICAL RULES:
1. Hit the word count targets: what_it_does must be ~500 words minimum (3-4 paragraphs, do not cut short). who_is_behind_it, what_is_notable, risks_to_monitor, and market_opportunity must each be ~125 words minimum. Do not truncate any section.
